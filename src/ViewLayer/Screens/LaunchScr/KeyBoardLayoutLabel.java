...
class KeyBoardLayoutLabel extends JLabel
{
     ...
     * This is used on tha launch screen to aid the user about the game controls.
     *
     * @param PlayerNumber The player's number (1 or 2). Different image will be loaded based on it.
     */
    public KeyBoardLayoutLabel(int PlayerNumber)
    {
        _BackGround = ImageFileLoader.LoadKeyboardLayoutImage(PlayerNumber);
        this.setSize(SharedResources.CLI_Width,SharedResources.CLI_Height);

        this.setBackground(SharedResources.CLI_Background_Color);
        this.setOpaque(true);

        String labelText = "Player " + Integer.toString(PlayerNumber) + " keys:";
